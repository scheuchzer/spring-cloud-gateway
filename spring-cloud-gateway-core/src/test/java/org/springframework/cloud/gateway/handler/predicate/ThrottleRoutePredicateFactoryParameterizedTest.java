...
public class ThrottleRoutePredicateFactoryParameterizedTest {
    ...
    public void throttle() {
        final Predicate<ServerWebExchange> predicate = new ThrottleRoutePredicateFactory().apply(rate);
        final long matchedCound = LongStream.range(0, totalCalls).filter(x -> predicate.test(exchange)).count();
