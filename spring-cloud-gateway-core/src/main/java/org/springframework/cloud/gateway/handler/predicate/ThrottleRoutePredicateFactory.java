...
/**
 * The throttle predicate allows a certain rate of requests to pass. The rate is a double between 0 and 1.
 * Example: to allow 25% of the traffic set a rate of 0.25.
 *
...
public class ThrottleRoutePredicateFactory implements RoutePredicateFactory {
    ...
    public Predicate<ServerWebExchange> apply(final double rate) {
        if (rate < 0.0 || rate > 1.0) {
            throw new ValidationException("Rate must be between 0 and 1 (both inclusive)");
        }
        ...
        final long greatestCommonDivisor = BigInteger.valueOf(divisor).gcd(BigInteger.valueOf(moduloThreshold))
            .longValue();
        return new ThrottlePredicate(
    ...
    public static class ThrottlePredicate implements Predicate<ServerWebExchange> {
        ...
        // if the atomic long counter becomes a bottleneck an alternative would be a LongAdder
        // but the adder doesn't have a incrementAndGet. Therefore race conditions could occur. But maybe
        // a fast counter with eventually wrong values is acceptable in this case.
